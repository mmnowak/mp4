...
def reviews(request, product_id):
    ...
    context = {"reviews": reviews, "product": product, "on_reviews_page": True}
...
def add_review(request, product_id):
    ...
    if not request.user.is_authenticated:
        messages.error(request, "Sorry, you need to log in to add a review!")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            avg_rating = round(
                Review.objects.filter(product=product.id).aggregate(
                    Avg("rating"))[
                    "rating__avg"
                ]
            )
...
def edit_review(request, review_id):
    ...
    if not request.user.is_authenticated:
        messages.error(request, "Sorry, you need to log in to edit a review!")
        return redirect(reverse("account_login"))
